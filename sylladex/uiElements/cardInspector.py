...
class CardInspector(UIElement):
    def __init__(self, code_data):
        super().__init__(settings.SCREEN_WIDTH-343, settings.SCREEN_HEIGHT /
                         2 - 219.5, (343, 447), 'CardInspector', (209, 158, 255))
        ...
        self.create_appearance(
            ...
            texts=[
                ...
                [codeDatabase.get_weaponkind_type(self.code_data.kind), [
                    278, 83], "center"],
        ...
        self.add_child(UIElement.get_ui_elem(
            'CardInspectorCheck')(81, 71, 'Trait1'))
        self.add_child(UIElement.get_ui_elem(
            'CardInspectorCheck')(81, 96, 'Trait2'))
        self.add_child(UIElement.get_ui_elem(
            'CardInspectorCheck')(27, 232, 'Action1'))
        self.add_child(UIElement.get_ui_elem(
            'CardInspectorCheck')(27, 257, 'Action2'))
        self.add_child(UIElement.get_ui_elem(
            'CardInspectorCheck')(159, 232, 'Action3'))
        self.add_child(UIElement.get_ui_elem(
            'CardInspectorCheck')(159, 257, 'Action4'))
        ...
        codeDatabase.get_action_image(
            self.code_data.action_3, self, [183, 232])
        codeDatabase.get_action_image(
            self.code_data.action_4, self, [183, 257])

        _kind_icon = pg.image.load(codeDatabase.find_kind_image(
            self.code_data.kind)).convert_alpha()
        _kind_icon = pg.transform.scale(_kind_icon, [24, 24])
        self.image.blit(_kind_icon, [92, 121])

        _grist_icon = pg.image.load(codeDatabase.find_grist_image(
            self.code_data.grist)).convert_alpha()
        ...
        for _index, _eff in enumerate(codeDatabase.gristData.get(self.code_data.grist).get('Effective')):
            _eff_icon = pg.image.load(
                codeDatabase.find_grist_image(_eff)).convert_alpha()
        ...
        for _index, _diseff in enumerate(codeDatabase.gristData.get(self.code_data.grist).get('Diseffective')):
            _diseff_icon = pg.image.load(
                codeDatabase.find_grist_image(_diseff)).convert_alpha()
        ...
        for _check in UIElement.get_ui_elem('CardInspectorCheck').checks:
            if _check.selected == True:
                if _check.job == 'Trait1' and self.prev_desc_type != 'Trait1':
                    ...
                    _trait_desc = codeDatabase.get_trat_1_data(
                        self.code_data.trait_1, self.code_data.kind, int(self.code_data.tier)).split(' ')
                    _lines = ['']
                    ...
                    _trait_desc = codeDatabase.get_trat_2_data(
                        self.code_data.trait_2, self.code_data.kind, int(self.code_data.tier)).split(' ')
                    _lines = ['']
    ...
    def change_description(self, new_desc, line_count, is_action=False, action_data=None):
        ...
        if _no_action == False:
            codeDatabase.get_action_image(
                self.code_data.action_1, self, [51, 232])
            codeDatabase.get_action_image(
                self.code_data.action_2, self, [51, 257])
            codeDatabase.get_action_image(
                self.code_data.action_3, self, [183, 232])
            codeDatabase.get_action_image(
                self.code_data.action_4, self, [183, 257])
