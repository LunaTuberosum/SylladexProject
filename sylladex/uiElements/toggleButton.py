...
class ToggleButton(UIElement):
    ...
    def hover(self):
        if self.on == False:
            self.background_color = pg.Surface((24,24))
            self.background_color.fill('#1C4587')
            self.image.blit(self.background_color, [6, 6])
            
            self.foreground_color = pg.Surface((24,24))
            self.foreground_color.fill('#C9DAF8')
            self.image.blit(self.foreground_color, [0, 0])
    ...
    def no_hover(self):
        if self.on == False:
            ...
            self.foreground_color = pg.Surface((24,24))
            self.foreground_color.fill('#3C78D8')
            self.image.blit(self.foreground_color, [0, 0])
            txt_surf = self.font.render(self.text, True, (0,0,0))
            self.image.blit(txt_surf, [12-(txt_surf.get_width()/2), 12-(txt_surf.get_height()/2)])
            self.hovering = False
